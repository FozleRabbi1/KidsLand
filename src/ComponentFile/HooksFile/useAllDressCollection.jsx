...
const useAllDressCollection = (getDataObj, selectedOption, value, selectedOption2, text) => {
    const { data: datas = [], refetch, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`http://localhost:5000/allDressCollection?itemOffset=${getDataObj.itemOffset}&endOffset=${getDataObj.endOffset}&selectedOption=${selectedOption}&lowRangeValue=${value[0]}&highRangeValue=${value[1]}&ascenDescen=${selectedOption2}&material=${text}`)
